      ...
      <view class="section-card">
        ...
        <view class="info-grid">
          <view class="info-item">
            ...
            <text class="info-value">{{ detail.caregiverName }}</text>
          </view>
          <view class="info-item">
            <text class="info-label">身份证号</text>
            <text class="info-value">{{ detail.idCard || '暂无' }}</text>
          </view>
          <view class="info-item">
            <text class="info-label">联系电话</text>
            <text class="info-value">{{ detail.phone || '暂无' }}</text>
          </view>
          <view class="info-item">
            <text class="info-label">从业年限</text>
            <text class="info-value">{{ detail.workYears ? detail.workYears + '年' : '暂无' }}</text>
          </view>
          <view class="info-item full-width">
            <text class="info-label">技能</text>
            <text class="info-value">{{ detail.skills || '暂无' }}</text>
          </view>
          <view class="info-item full-width">
...
async function loadDetail() {
  ...
  try {
    await adminStore.fetchCaregiverReviewDetail(caregiverId.value)
    detail.value = adminStore.currentCaregiverReview
  } catch (err) {
...
async function handleApprove() {
  uni.showModal({
    title: '确认操作',
    content: '确定通过该护理人员的审核吗？',
    success: async (res) => {
      if (res.confirm) {
        const success = await adminStore.auditCaregiver(caregiverId.value, 'APPROVED', reviewComment.value)
        if (success) {
          uni.showToast({
            title: '审核通过',
            icon: 'success',
...
async function handleReject() {
  ...
  uni.showModal({
    title: '确认操作',
    content: '确定驳回该护理人员的审核吗？',
    success: async (res) => {
      if (res.confirm) {
        const success = await adminStore.auditCaregiver(caregiverId.value, 'REJECTED', reviewComment.value)
        if (success) {
          uni.showToast({
            title: '已驳回',
            icon: 'success',
...
onMounted(() => {
  ...
  caregiverId.value = currentPage.options?.id
