    ...
    <view v-else-if="detail" class="detail-content">
      ...
      <view class="section-card">
        ...
        <view class="info-grid">
          ...
          <view class="info-item">
            ...
            <text class="info-value">{{ detail.categoryName || '暂无' }}</text>
          </view>
          <view class="info-item">
            <text class="info-label">服务价格</text>
            <text class="info-value price">{{ detail.price ? '¥' + detail.price : '暂无' }}</text>
          </view>
          <view class="info-item">
            <text class="info-label">服务时长</text>
            <text class="info-value">{{ detail.duration ? detail.duration + '分钟' : '暂无' }}</text>
          </view>
          <view class="info-item full-width">
            <text class="info-label">所属商户</text>
            <text class="info-value">{{ detail.merchantName || '暂无' }}</text>
          </view>
        </view>
      </view>
      ...
      <view class="section-card">
        ...
        <view class="detail-content-wrapper">
          <text class="detail-text">{{ detail.description || '暂无服务介绍' }}</text>
        </view>
      </view>

      <!-- 服务规格 -->
      <view v-if="detail.specs && detail.specs.length > 0" class="section-card">
        <view class="section-title">
          <text class="title-icon">📦</text>
          <text class="title-text">服务规格</text>
        </view>
        <view class="specs-list">
          ...
          >
            ...
            <view v-if="spec.description" class="spec-desc">
      ...
      <!-- 服务图片 -->
      <view v-if="detail.images && detail.images.length > 0" class="section-card">
        <view class="section-title">
...
async function loadDetail() {
  ...
  try {
    await adminStore.fetchServiceReviewDetail(serviceId.value)
    detail.value = adminStore.currentServiceReview
  } catch (err) {
...
async function handleApprove() {
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        const success = await adminStore.auditService(serviceId.value, 'APPROVED', reviewComment.value)
        if (success) {
          uni.showToast({
            title: '审核通过',
            icon: 'success',
...
async function handleReject() {
  ...
  uni.showModal({
    ...
    success: async (res) => {
      if (res.confirm) {
        const success = await adminStore.auditService(serviceId.value, 'REJECTED', reviewComment.value)
        if (success) {
          uni.showToast({
            title: '已驳回',
            icon: 'success',
