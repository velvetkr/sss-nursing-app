<template>
  <view v-if="order" class="page-shell">
    ...
    <view class="bottom-actions">
      ...
      <button v-if="order.status === 2" class="action-btn" @click="complaint">投诉反馈</button>
...
async function pay() { await orderStore.payOrder(order.value.orderId); uni.redirectTo({ url: `/pages/payment-result/payment-result?status=success&orderId=${order.value.orderId}&amount=${order.value.totalAmount}` }) }
