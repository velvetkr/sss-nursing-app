  ...
  <view class="page-shell">
    ...
    <view v-else class="order-list">
      <view v-for="order in orderStore.orders" :key="order.orderId" class="order-card" @click="goDetail(order.orderId)">
        <view class="card-head"><text class="order-no">订单 {{ order.orderNo }}</text><text class="status" :class="`status-${order.status}`">{{ orderStore.getStatusText(order.status) }}</text></view>
